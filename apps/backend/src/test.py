...
def fetch_data_from_postgres():
    ...
    # Initialize lists
    ids, descriptions, names, embeddings = [], [], [], []
    ...
    while True:
        ...
        for row in batch:
            if row[3] is not None:
                ids.append(row[0])
                descriptions.append(row[1] if row[1] else '')
                names.append(row[2] if row[2] else '')
    ...
    # Convert embeddings to numpy matrix with progress bar
