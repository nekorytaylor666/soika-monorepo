...
export async function performSearch(
	...
	const { limit, offset, minBudget, maxBudget, tradeMethod, withRecommendations } = options;
	...
	const keywordResultsPromise = db.query.lots.findMany({
		where: (table, { inArray }) =>
			and(
				withRecommendations ? inArray(table.id, db.select({ lotId: recommendedProducts.lotId }).from(recommendedProducts)) : undefined,
				...
				tradeMethod ? sql`ref_trade_methods ->> 'id' = ${tradeMethod}` : undefined,
			),
		with: {
			recommendedProducts: {
				columns: {
	...
	const [embedding, keywordResults] = await Promise.all([embeddingPromise, keywordResultsPromise]);
	...
	const combinedResults = Array.from(new Set([...semanticResults, ...keywordResults]));
...
async function rerankResults(query: string, documents: any[], limit: number) {
	try {
		...
		if (!jinaResponse.ok) {
			throw new Error(`Jina API request failed with status ${jinaResponse.status}`);
		}
