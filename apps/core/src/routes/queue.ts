...
const queue = async (
	batch: MessageBatch<MessageType<typeof EVENT_TYPE.parseProducts | typeof EVENT_TYPE.lotCreation>>,
...
const handleRecommendation = async (body: MessageType<typeof EVENT_TYPE.parseProducts>, env: HonoEnv['Bindings'], message: Message) => {
	...
	if (!extraction.isTender) {
		...
		const compliance = await evaluateProductCompliance(extraction.specifications, lot.lotSpecifications ?? '');
...
const handleLotCreation = async (body: MessageType<typeof EVENT_TYPE.lotCreation>, env: HonoEnv['Bindings'], message: Message) => {
