...
import { organizationMembers, scheduleResults, schedules as scheduleSchema, type ScheduleFrequency } from 'db/schema/schema';
...
export async function performScheduledSearch(scheduleId: string) {
	...
	await db.insert(scheduleResults).values({ scheduleId, result, createdAt: new Date(), organizationId: schedule.organizationId });
