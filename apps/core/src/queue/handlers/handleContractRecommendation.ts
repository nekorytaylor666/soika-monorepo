...
async function extractTechnicalSpecification(
  contractId: string,
  contractUnit: string,
) {
  ...
  const response = await axios.post(
    url,
    `pid=${contractId}&unit_id=${contractUnit}`,
    {
      headers: {
  ...
  const techSpecLink = document
    .querySelector('a[href*="techspec"]')
    ?.getAttribute("href");
...
export async function handleContractRecommendation(body: { lotId: string }) {
  ...
  const techSpecLink = await extractTechnicalSpecification(
    bestMatch?.id?.toString(),
    bestMatch.contractUnit?.id?.toString() || "",
  );
