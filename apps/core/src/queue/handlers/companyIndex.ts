...
async function ingestData(bin: string): Promise<void> {
	...
	for (let i = 0; i < lotEmbeddings.length; i += insertBatchSize) {
		...
		try {
			...
			console.log(`Inserted ${i + batch.length} of ${lotEmbeddings.length} embeddings. Insert result:`);
		} catch (error) {
	...
	console.log(`Total inserted: ${lotEmbeddings.length} lot embeddings into Milvus for BIN: ${bin}`);
}
...
async function searchSimilarLots(queryTexts: string[], bin: string, topK: number = 20): Promise<any[]> {
...
async function fetchNewLots(): Promise<any[]> {
  ...
  `;
	...
	try {
		const response = await axios.post(url, { query }, { headers, withCredentials: true });
...
async function generateRecommendationReport(bin: string): Promise<any[]> {
	...
	const batchSize = 200;
	const res = await milvusClient.loadPartitions({ collection_name: 'tender_lots', partition_names: [bin] });
	console.log(res);
	...
	for (let i = 0; i < newLots.length; i += batchSize) {
		...
		const searchQueries = batch.map((lot) => `${lot.nameRu} ${lot.descriptionRu || ''}`.trim());
	...
	await milvusClient.releasePartitions({ collection_name: 'tender_lots', partition_names: [bin] });

	return lotsWithSimilarResults;
}

export async function handleCompanyIndex(data: MessageType<typeof EVENT_TYPE.companyIndex>) {
