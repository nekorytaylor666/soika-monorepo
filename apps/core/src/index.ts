...
import { router } from './queue/jobRouter';
...
app.post('/compliance/:productId', async (c) => {
	...
	const result = await evaluateProductCompliance(product.productSpecifications, product.lot.lotSpecifications);
	return c.json({ result });
});

app.post('/clean_lot_backlog', async (c) => {
	const allLots = await db.query.plagiarismCheck.findMany();
	const urlLots = allLots.filter((el) => el.urls?.length && el.unique !== null && el.unique < 70);
	const limitedUrlLots = urlLots.slice(-50); // Changed to select the last 50 elements
	for (const lot of limitedUrlLots) {
		await c.env.soikaQueue.send({ lotId: lot.id, type: EVENT_TYPE.parseProducts });
	}
